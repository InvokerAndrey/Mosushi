      ...
      <div className={`md:hidden overflow-hidden transition-all duration-300 border-t-2 border-zinc-900 ${mobileMenuOpen ? "max-h-[400px] opacity-100" : "max-h-0 opacity-0"}`}>
        <nav className="flex flex-col bg-[#f3f5eb] dark:bg-zinc-950">
          {["sushi", "sets", "sauces", "drinks", "checkout"].map((section, index) => (
            <a
              key={section}
              className={`px-8 py-4 ${index === 0 ? "text-primary-container" : "text-zinc-600 dark:text-zinc-400"} ${index < 4 ? "border-b border-zinc-300 dark:border-zinc-700" : ""} hover:bg-zinc-900 hover:text-white transition-all duration-200`}
              href={`#${section}`}
              onClick={(e) => { e.preventDefault(); handleMobileMenuClick(section); }}
            >
              {section.toUpperCase()}
            </a>
          ))}
