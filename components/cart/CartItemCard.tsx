      ...
      <div className="flex items-center gap-3 flex-shrink-0">
        <div className="flex border border-on-background w-[80px]">
          <button 
            type="button"
            onClick={onDecrease}
            className="px-2 py-1 hover:bg-zinc-200 text-xs w-7 text-center"
            aria-label={`Decrease ${item.name} quantity`}
          >
            -
          </button>
          <span className="px-3 py-1 font-product-name text-xs border-l border-r border-on-background w-[26px] text-center">
            {item.quantity}
          </span>
          <button 
            type="button"
